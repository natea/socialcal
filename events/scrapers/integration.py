...
async def scrape_and_save_events(scraper_url: str, scraper_name: str = None) -> List[Event]:
    """
    Scrape events using our generic extractor and save them to the database.
    
    Args:
        scraper_url: The URL to scrape
        scraper_name: Optional name for the scraper
        
    Returns:
        A list of created or updated Event instances
    """
    logger.info(f"Scraping events from URL: {scraper_url}")
    
    # Use our generic event extractor
    events_data = await scrape_events(scraper_url)
    ...
    if not scraper_name:
        # Generate a name from the URL domain
        from urllib.parse import urlparse
        domain = urlparse(scraper_url).netloc
        scraper_name = f"Event Scraper - {domain}"
...
async def process_all_scrapers():
    """Process all site scrapers in the database."""
    # Get all scrapers
    scrapers = SiteScraper.objects.all()
    
    if not scrapers:
        logger.warning("No scrapers found in the database.")
        return
    
    logger.info(f"Found {scrapers.count()} scrapers to process")
    
    # Process each scraper
    total_events = 0
    for scraper in scrapers:
        events = await scrape_and_save_events(scraper.url, scraper.name)
...
async def main():
    ...
    logger.info("Starting event integration")
    
    # If a URL is provided as a command-line argument, process just that URL
    if len(sys.argv) > 1:
        url = sys.argv[1]
        logger.info(f"Processing single URL: {url}")
        await scrape_and_save_events(url)
    else:
        # Process all scrapers
        await process_all_scrapers()
    
    logger.info("Event integration completed")
